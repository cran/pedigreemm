...
SEXP pedigree_inbreeding(SEXP x)
{
	...
	*sire = INTEGER(sp),
	*dam = INTEGER(GET_SLOT(x, install("dam")));
    double *F = calloc(n + 1, sizeof(double)), /* inbreeding coefficients */
      *L = calloc(n + 1, sizeof(double)), *B = calloc(n + 1, sizeof(double));
    int *Anc = calloc(n + 1, sizeof(int)),	/* ancestor */
	*LAP = calloc(n + 1, sizeof(int)); 	/* longest ancestoral path */
    R_CheckStack();
    ...
    SI = calloc(t + 1, sizeof(int));
    MI = calloc(t + 1, sizeof(int));
